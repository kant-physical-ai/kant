...
- P (비례): 지금 오차 크기에 비례 → kp * error_rad (find-gain.ino:265)
- I (적분): 오차를 시간에 걸쳐 누적한 값에 비례 → 정상상태 오차(오래 남아있는 작은 오차)를 없애는 역할
- D (미분): 오차가 변하는 속도(=오차의 시간미분)에 비례 → 급하게 움직이는 걸 미리 억제해서 오버슈트/진동을 줄이는 역할 (브레이크 같은 느낌)

지금 find-gain.ino는 P만 구현돼 있고 I, D는 없습니다. kp 하나만 튜닝 대상이고, ki/kd 변수 자체가 코드에 없어요.
 */
...
void setup() {
  ...
  if (!requireOk(dxl.ping(DXL_ID), "ping: check ID/baud/power") ||
      !requireOk(dxl.getModelNumber(DXL_ID) == XM430_W210, "requires XM430_W210") ||
